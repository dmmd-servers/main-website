...
export class Event<Table extends Record<string, (...parameters: any[]) => void | Promise<void>>> {
    // Defines field
    private readonly _table: Partial<Record<keyof Table, Table[keyof Table][]>> = {};
    ...
    off<Name extends keyof Table>(
        ...
        if(list.length === 0) delete this._table[name];
    }
    on<Name extends keyof Table>(
        ...
        // Appends listener
        const list = this._table[name];
        if(typeof list === "undefined") this._table[name] = [ listener ];
